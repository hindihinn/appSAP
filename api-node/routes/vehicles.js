...
router.get('/stats/by-date', auth, async (req, res) => {
  try {
    ...
    if (!fromDate) {
      return res.status(400).json({ success: false, message: 'Parameter date atau date_from diperlukan' });
    }

    const query = `
      SELECT v.*, u.name as unit_name, c.name as company_name, c.id as company_id,
        CASE 
    ...
    const [rows] = await db.query(query, [toDate, fromDate]);
    res.json({ success: true, data: rows });
  } catch (error) {
...
// GET driver yang di-assign ke kendaraan tertentu (active assignment) — MUST be before /:id

router.get('/:id/assigned-driver', auth, async (req, res) => {
  try {
    const [rows] = await db.query(
      `SELECT d.id, d.name, d.employee_id, d.phone, d.status,
    ...
    if (rows.length === 0) {
      return res.json({ success: true, data: null, message: 'Tidak ada driver yang di-assign ke kendaraan ini' });
    }
